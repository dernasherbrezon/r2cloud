...
public class R2Cloud {
	...
	public R2Cloud(Configuration props) {
		...
		decoders.put("39430", new Gomx1Decoder(props, predict));
		decoders.put("39444", new Ao73Decoder(props, predict));
		decoders.put("40069", new LRPTDecoder(props, predict));
		decoders.put("41460", new Aausat4Decoder(props, predict));
		decoders.put("42017", new Nayif1Decoder(props, predict));
		decoders.put("42784", new PegasusDecoder(props, predict));
		decoders.put("42829", new TechnosatDecoder(props, predict));
		SnetDecoder snetDecoder = new SnetDecoder(props, predict);
		...
		decoders.put("43743", new ReaktorHelloWorldDecoder(props, predict));
		decoders.put("43792", new EseoDecoder(props, predict));
		decoders.put("43798", new AstrocastDecoder(props, predict));
		decoders.put("43803", new Jy1satDecoder(props, predict));
		decoders.put("43804", new Suomi100Decoder(props, predict));
		...
		decoders.put("44387", new MeteorM22Decoder(props, predict, processFactory));
